...
std::shared_ptr<settings> cg::settings::parse_settings(int argc, char** argv)
{
	...
	add_options("model_path", "Path to OBJ model", cxxopts::value<std::filesystem::path>()->default_value("models/CornellBox-Original.obj"));
	add_options("camera_position", "Camera position", cxxopts::value<std::vector<float>>()->default_value("0.0,1.0,2.0"));
	...
	add_options("raytracing_depth", "Maximum number of traces rays", cxxopts::value<unsigned>()->default_value("16"));
	add_options("accumulation_num", "Number of accumulated frames", cxxopts::value<unsigned>()->default_value("16"));
